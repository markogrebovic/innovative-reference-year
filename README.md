...
* **Locations / Climate Zones (Montenegro):**
  ...
  * `PV` – Pljevlja (Humid continental climate, elevation: 784 m)[cite: 1].
* **Features Included:** Daily mean temperature (`TEMP`), daily maximum temperature (`MAX`), daily minimum temperature (`MIN`), relative humidity (`RHUM`, converted from dew point `DEWP`), precipitation (`PRCP`), visibility (`VSB`), daily mean wind speed (`WDSP`), and maximum wind speed (`MXSPD`)[cite: 1].
* **Data Splitting:**
  * **FS Method & DL Training/Validation:** 2002–2023[cite: 1].
  * **Model Testing / Verification:** 2024 (Leap day Feb 29 removed for 365-day compatibility)[cite: 1].

## Code Information
The code base contains end-to-end Python scripts for climate data preprocessing, standard statistical generation, deep learning sequence modeling, and building dynamic thermal analysis[cite: 1].

* **FS Method Implementation:** Implements standardized CDF calculations and month selection per EN ISO 15927-4[cite: 1].
* **Deep Sequence Architectures:**
  * 1-layer and 2-layer LSTM / GRU models with layer sizes of 32, 64, and 128 neurons[cite: 1].
  * Hybrid Conv1D-RNN networks using 1D temporal convolution (kernel size $k=3$, stride $s=2$) for spatial-temporal downsampling[cite: 1].
  * Input & Output sequences: 365 time steps (predicting a full annual cycle)[cite: 1].
* **Thermal Assessment Module:** Dynamic heat transfer matrix formulation according to EN ISO 13786 and EN ISO 52016-1 for calculating $Q_h$ (heating) and $Q_c$ (cooling) flux densities[cite: 1].
